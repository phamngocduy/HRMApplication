...
(function ($) {
    ...
    $.fn.rowReordering = function (options) {
        ...
        return this.each(function () {
            ...
            var aoRows = $("tbody tr", oTable);
            for (var id=0; id<aoRows.length; id++)
                $(aoRows[id]).attr("no", oSettings.aiDisplayMaster[id]);
            oTable.fnSettings()["aoDrawCallback"].push({
                "sName": "RowReorder_Draw",
                "fn": function(oSettings) {
                    var aoRows = $(oSettings.nTable).attr("no").split(",");
                    if (aoRows.length == oSettings.aiDisplayMaster.length)
                        for (var id=0; id<aoRows.length; id++)
                            oSettings.aiDisplayMaster[id] = aoRows[id];
                }
            });
            oTable.fnSettings()["aoStateSaveParams"].push({
                "sName": "RowReorder_State",
                "fn": function (oSettings, oState) {
                    if (oSettings.aiDisplay.length == oSettings.aiDisplayMaster.length) {
                        oState.RowReorder = [];
                        var aoRows = $("tbody tr", oSettings.nTable);
                        for (var id=0; id<aoRows.length; id++) {
                            var no = $(aoRows[id]).attr("no");
                            if (no == undefined) {
                                oState.RowReorder.push(oSettings.aiDisplayMaster[id]);
                                $(aoRows[id]).attr("no", oSettings.aiDisplayMaster[id]);
                            } else oState.RowReorder.push(no);
                        }
                        $(oSettings.nTable).attr("no", oState.RowReorder);
                    }
                    else
                        oState.RowReorder = $(oSettings.nTable).attr("no").split(",");
                }
            });
            var aiOrder = oSettings.oLoadedState;
            if (aiOrder)
                aiOrder = oSettings.oLoadedState.RowReorder;
            if (aiOrder)
                if (aiOrder.length == oSettings.aiDisplayMaster.length)
                    for (var i=0; i<aiOrder.length; i++)
                        oSettings.aiDisplayMaster[i] = aiOrder[i];
            $(oTable).attr("no", oSettings.aiDisplayMaster);
            oTable.fnSettings().oFeatures.bSort = false;
            oTable.fnDraw();
            ...
            $("tbody", oTable).disableSelection().sortable({
                ...
                update: function (event, ui) {
                    ...
                    oTable._fnSaveState(oTable.fnSettings());
                    /*
